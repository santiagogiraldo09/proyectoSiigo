...
def procesar_excel_para_streamlit(uploaded_file):
    ...
    try:
        ...
        if "Tasa de cambio" in df_procesado.columns and "Observaciones" in df_procesado.columns:
            ...
            # Asegura que la columna 'Observaciones' sea de tipo texto
            ...
            # Crea una "máscara" para identificar las filas donde SÍ se encontró un valor
            mascara_actualizacion = pd.notna(trm_extraida)

            # Usa .loc para actualizar 'Tasa de cambio' SÓLO en las filas donde la máscara es Verdadera
            df_procesado.loc[mascara_actualizacion, 'Tasa de cambio'] = trm_extraida[mascara_actualizacion]
            
            filas_actualizadas = mascara_actualizacion.sum()
